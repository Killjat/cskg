...
def test_phone_uploads():
    """测试手机上传的图片"""
    print("=== 测试手机上传的图片GPS位置信息提取 ===")
    ...
    for filename in os.listdir(upload_dir):
        if filename.lower().endswith((".jpg", ".jpeg", ".png", ".tiff", ".tif")):
            # 只选择大于100KB的文件（手机上传的图片通常较大）
            file_path = os.path.join(upload_dir, filename)
            file_size = os.path.getsize(file_path)
            if file_size > 100 * 1024:  # 大于100KB
                image_files.append(filename)
    
    if not image_files:
        print(f"上传目录 {upload_dir} 中没有手机上传的图片（大于100KB的图片）")
        return
    
    print(f"手机上传的图片数量: {len(image_files)}")
